...
Page({
  ...
  storeInput: function() {
    ...
    }else if(this.data.userInputchips<8000){
      wx.showToast({
        title: '请输入不少于8000的筹码数',
        icon: 'none',  // 为 'none' 时，不显示图标只显示文字
        duration: 2000  // 提示的延迟时间，单位毫秒，默认：1500
      });
    }else{
      wx.redirectTo({
        url: `/pages/funpvp/funpvp?userInputgames=${this.data.userInputgames}&userInputchips=${this.data.userInputchips}`
      });
    }
